...
def start_bot_in_background():
    ...
    try:
        ...
        if discord_token:
            try:
                from gateway_discord_bot import run_gateway_discord_bot
                
                def run_discord():
                    try:
                        logger.info("Starting Gateway Discord bot thread...")
                        asyncio.run(run_gateway_discord_bot(trading_bot, discord_token))
                    except Exception as e:
                ...
                logger.info("Gateway Discord bot thread started in background")
            except ImportError as e:
                logger.error(f"Failed to import Gateway Discord bot: {e}")
